"""
combined_loss.py - Kết hợp nhiều loại loss function
"""
import torch
...
class CrossEntropyLoss(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            lengths: Độ dài thực của mỗi chuỗi, kích thước [batch_size]
        ...
        # Reshape logits cho cross entropy
        ...
        if lengths is not None:
            ...
            loss = loss_sum / lengths.float()  # [batch_size]
            loss = loss.mean()
        else:
...
class CTCLoss(nn.Module):
    ...
    def forward(
        ...
        logits_lengths: torch.Tensor,  # [batch_size]
        targets_lengths: torch.Tensor  # [batch_size]
    ) -> torch.Tensor:
        """
        ...
        Args:
            ...
            logits_lengths: Độ dài của logits, kích thước [batch_size]
            targets_lengths: Độ dài của targets, kích thước [batch_size]
        ...
        # Tính CTC loss
        loss = self.criterion(logits, targets, logits_lengths, targets_lengths)
...
class CombinedLoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.cross_entropy = CrossEntropyLoss(
            label_smoothing=self.config.get('loss.label_smoothing', 0.1)
        )
        
        self.ctc_loss = CTCLoss()
        
        self.focal_loss = FocalLoss(
            alpha=self.config.get('loss.focal_alpha', 0.25),
            gamma=self.config.get('loss.focal_gamma', 2.0)
        )
        
        # Trọng số cho mỗi loss
        self.cross_entropy_weight = self.config.get('loss.cross_entropy_weight', 1.0)
        self.ctc_weight = self.config.get('loss.ctc_weight', 0.5)
        self.focal_weight = self.config.get('loss.focal_weight', 0.5)
        ...
        self.hard_mining_ratio = self.config.get('loss.hard_mining_ratio', 0.2)
    ...
    def forward(
        ...
        if self.loss_type == 'cross_entropy':
            if self.hard_mining:
                ...
                loss_per_sample = self.cross_entropy(combined_logits, ce_targets, targets_lengths - 1)
                return self._apply_hard_mining(loss_per_sample, self.hard_mining_ratio)
            return ce_loss
        
        elif self.loss_type == 'ctc':
            ctc_loss = self.ctc_loss(logits, targets, logits_lengths, targets_lengths)
            if self.hard_mining:
                return ctc_loss  # CTC loss đã là loss cho toàn batch
            return ctc_loss
            ...
            if self.hard_mining:
                # Tính loss cho từng mẫu
                loss_per_sample = self.focal_loss(combined_logits, ce_targets, targets_lengths - 1)
                return self._apply_hard_mining(loss_per_sample, self.hard_mining_ratio)
            return focal_loss
        ...
        # Nếu sử dụng combined loss
            ...
            ctc_loss = self.ctc_loss(logits, targets, logits_lengths, targets_lengths) if self.ctc_weight > 0 else 0
...
def create_loss_function() -> nn.Module:
    ...
    if loss_type == 'cross_entropy':
        ...
        return FocalLoss(
            alpha=config.get('loss.focal_alpha', 0.25),
            gamma=config.get('loss.focal_gamma', 2.0)
        )
    else:  # combined
        return CombinedLoss()