"""
config.py - Cấu hình cho mô hình và quá trình huấn luyện (đã sửa lỗi character vocabulary)
"""
import os
import yaml
...
class Config:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config = {
            ...
            'data': {
                ...
                'val_ratio': 0.1,
                'test_ratio': 0.1,
                'batch_size': 64,  # Tăng batch size để training nhanh hơn
                'num_workers': 4,
                'image_size': (32, 140),  # (height, width)
                'max_length': 10,  # Độ dài tối đa chuỗi ký tự
                # SỬA LỖI: Thêm các ký tự đặc biệt vào vocabulary
                'chars': '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ-+.',  # Thêm '+' và '.'
            },
            ...
            # Cấu hình huấn luyện
            'training': {
                'epochs': 50,  # Giảm số epochs để test nhanh hơn
            ...
            'loss': {
                ...
                'ctc_weight': 0.0,  # Tắt CTC để tránh lỗi
                'hard_mining': False,  # Tắt hard mining
    ...
    def _post_process_config(self) -> None:
        ...
        # Loại bỏ ký tự trùng lặp và sắp xếp
        unique_chars = ''.join(sorted(set(chars)))
        
        self.config['data']['char_to_idx'] = {char: idx + 3 for idx, char in enumerate(unique_chars)}  # +3 cho special tokens
        self.config['data']['idx_to_char'] = {idx + 3: char for idx, char in enumerate(unique_chars)}
        
        # Thêm special tokens
        self.config['data']['char_to_idx']['<pad>'] = 0
        self.config['data']['char_to_idx']['<sos>'] = 1 
        self.config['data']['char_to_idx']['<eos>'] = 2
        self.config['data']['idx_to_char'][0] = '<pad>'
        self.config['data']['idx_to_char'][1] = '<sos>'
        self.config['data']['idx_to_char'][2] = '<eos>'
        
        # Cập nhật num_classes bao gồm cả special tokens
        self.config['data']['num_classes'] = len(unique_chars) + 3  # +3 cho <pad>, <sos>, <eos>
        
        # In thông tin debug
        print(f"Character vocabulary: '{unique_chars}'")
        print(f"Total vocabulary size: {len(unique_chars)} + 3 special tokens = {self.config['data']['num_classes']}")
