"""
train_optimized.py - Script huấn luyện tối ưu cho dataset lớn
"""
import os
import sys
import time
...
def normalize_license_plate(text: str) -> str:
    """Normalize license plate format để so sánh"""
    if not isinstance(text, str):
        text = str(text)
    
    # Loại bỏ các ký tự đặc biệt và chuyển về uppercase
    normalized = text.replace('-', '').replace('+', '').replace('.', '').replace(' ', '').upper().strip()
    return normalized


def decode_predictions_fast(model, images, config):
    """Fast prediction decoding"""
    try:
        with torch.no_grad():
            ...
            for i in range(predicted_indices.size(0)):
                ...
                # Normalize format
                prediction = normalize_license_plate(prediction)
...
def validate_epoch_fast(model, dataloader, criterion, device, config, logger, max_batches=50):
    """Validation nhanh với giới hạn số batches"""
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(progress_bar):
            ...
            try:
                ...
                # Fast prediction decoding
                predictions = decode_predictions_fast(model, images, config)
                
                # Decode targets
                if labels is not None:
                    target_strings = [normalize_license_plate(str(label)) for label in labels]
                else:
                    # Simplified target decoding
                    target_strings = []
                    idx_to_char = config.get('data.idx_to_char', {})
                    for i in range(targets.size(0)):
                        ...
                        for idx in indices:
                            if idx in [0, 1]:  # pad, sos
                                ...
                                chars.append(idx_to_char[idx])
                        target_string = normalize_license_plate(''.join(chars))
