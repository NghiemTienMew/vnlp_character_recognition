...
def collate_fn(batch):
    ...
    # Unpack tất cả 3 phần tử từ batch
    ...
    for target in targets:
        ...
        # Convert target to list if it's a tensor
        if torch.is_tensor(target):
            target = target.tolist()
        
        # Pad target to max_len
        if target_length < max_len:
            padded_target = target + [0] * (max_len - target_length)
        else:
    ...
    # Trả về images, targets, target_lengths (không cần labels cho training)
