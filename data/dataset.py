"""
dataset.py - Module xử lý dữ liệu và tạo DataLoader
"""
import os
import pandas as pd
import numpy as np
...
class LicensePlateDataset(Dataset):
    ...
    def _encode_label(self, label: str) -> torch.Tensor:
        ...
        indices = [self.char_to_idx.get(c, 0) for c in label]
        
        # Thêm token bắt đầu (<sos>) và kết thúc (<eos>)
        sos_idx = self.char_to_idx.get('<sos>', len(self.char_to_idx))
        eos_idx = self.char_to_idx.get('<eos>', len(self.char_to_idx) + 1)
        
        indices = [sos_idx] + indices + [eos_idx]
        
        # Padding để đảm bảo độ dài cố định
        pad_idx = self.char_to_idx.get('<pad>', len(self.char_to_idx) + 2)
